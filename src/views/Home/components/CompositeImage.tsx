...
export const getImageUrl = (base: string, imageSrc: string, resolution?: Resolution, extension = '.webp'): string =>
  `${base}${imageSrc}${resolution ? `@${resolution}${extension}` : extension}`

export const getSrcSet = (base: string, imageSrc: string, extension = '.webp') => {
...
const CompositeImage: React.FC<ComponentProps> = ({ path, attributes, maxHeight = '256px' }) => {
  return (
    <Wrapper maxHeight={maxHeight}>
      <DummyImg
        srcSet={getSrcSet(path, attributes[0].src, '.png')}
        maxHeight={maxHeight}
        loading="lazy"
        decoding="async"
        onError={(e) => {
      ...
      {attributes.map((image) => (
        <ImageWrapper key={image.src}>
          <img
            srcSet={getSrcSet(path, image.src, '.png')}
            alt={image.alt}
            onError={(e) => {
