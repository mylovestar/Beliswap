...
export const DEFAULT_META: PageMeta = {
  title: 'Beli Swap',
  description:
    'The most popular AMM on BSC by user count! Earn BEFX through yield farming or win it in the Lottery, then stake it in Syrup Pools to earn more tokens! Initial Farm Offerings (new token launch model pioneered by Beli Swap), NFTs, and more, on a platform you can trust.',
...
const getPathList = (t: ContextApi['t']): PathList => {
  return {
    ...
    defaultTitleSuffix: t('Beli Swap'),
  }
}
...
export const getCustomMeta = memoize(
  (path: string, t: ContextApi['t'], _: string): PageMeta => {
    ...
    if (pathMetadata) {
      ...
      // }
      return {
        title: `${t(pathList.defaultTitleSuffix)}`,
        ...(pathMetadata.description && { description: pathMetadata.description }),
      }
    }
    return null
