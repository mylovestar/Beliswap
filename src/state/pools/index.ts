...
export const fetchPoolsPublicDataAsync = (currentBlockNumber: number) => async (dispatch, getState) => {
  try {
    ...
    const activePriceHelperLpsConfig = priceHelperLpsConfig.filter((priceHelperLpsConfig) => {
      return (
        poolsConfig
          .filter((pool) => pool.earningToken.address.toLowerCase() === priceHelperLpsConfig.token.address.toLowerCase())
          .filter((pool) => {
